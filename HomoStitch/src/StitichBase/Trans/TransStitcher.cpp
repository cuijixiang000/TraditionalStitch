...
cv::Mat TransStitcher::SoBuild()
{
	LOG("********Start TransStitcher...********");
	CalFeatures();
	if (!ImgsMatch())
	{
		ERROR("ImgsMatch ERROR!!!");
		return cv::Mat();
	}
	///放入m_GroupImgs;
	SelectBaseFrame();
	///计算投影范围
	UpDateRange();
	LOG("********TransStitcher is OK!!!********");
	return  Blender();
}

void TransStitcher::SelectBaseFrame()
{
	///选择中间帧为基准
...
void TransStitcher::UpDateRange()
{
...
#pragma omp parallel for schedule(dynamic)
	for (int i = 0; i < iImgsNum; i++)
	{
		...
		//DEBUG("{}->{}: {}", iNext, i, ImgMathInfo.viMatchIndex.size());
	...
	cout << "ImgMathInfo.Homo:\n " << _vvMatchsInfo[0][1].Homo << endl;
	return true;
}

cv::Point2f TransStitcher::GetFinalResolusion()
{
